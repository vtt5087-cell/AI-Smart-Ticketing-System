...
pattern = r"// Check if we need to auto-respond to a user message.*?if \(updatedTicket\.aiAutoRespond !== false\) \{.*?const oldMsgsLength = existingTicket\.messages\?\.length \|\| 0;.*?const newMsgsLength = updatedTicket\.messages\?\.length \|\| 0;.*?if \(newMsgsLength > oldMsgsLength\) \{.*?const latestMsg = updatedTicket\.messages\[newMsgsLength - 1\];.*?if \(latestMsg\.sender === 'user' && !latestMsg\.text\.startsWith\('CUSTOMER_CSAT_RATING:'\)\) \{"
