...
# Because we changed the structure slightly, let's use replace instead of re
text = text.replace(target, replacement)

# Fix brace closing since we added a block
target2 = """            updatedTicket.messages = [...updatedTicket.messages, aiMessage];
          }
        } catch (err: any) {
...
replacement2 = """            updatedTicket.messages = [...updatedTicket.messages, aiMessage];
          }
        } catch (err: any) {
          console.error("Auto-respond failed:", err.message);
        }
